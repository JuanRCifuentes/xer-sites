...
P6 es principalmente una herramienta de programacion, pero tambien puede soportar cronogramas cargados con costos, earned value, cash flow y reportes de forecast. Para hacerlo correctamente, el costo debe ubicarse en la parte correcta del modelo.
...
La ubicacion del costo importa porque la fuente del costo afecta como se resume, actualiza, forecasted y reporta.
...
flowchart TD
    ...
    F --> H["Cash flow, earned value, reportes y forecasts"]
    G --> H
```
...
Remaining Cost es el costo forecasted que aun se necesita para completar la actividad. Esta vinculado a remaining units, resource rates, remaining expenses y supuestos de actualizacion.

Remaining Cost es uno de los campos de forecast mas importantes. Le dice al equipo cuanto costo queda desde la Data Date hacia adelante.

Use Remaining Cost para responder: cuanto costo todavia se espera?

Si Remaining Duration se actualiza pero Remaining Cost no, el forecast puede volverse inconsistente. Lo mismo ocurre cuando las resource units o remaining expenses no se mantienen.
...
At Completion Cost ayuda a mostrar si una actividad esta forecasted para terminar sobre, bajo o en presupuesto.
...
Baseline cost es importante para reportes de variacion. Sin baseline, el proyecto puede conocer el forecast actual, pero no si ese forecast esta mejor o peor que el plan aprobado.
