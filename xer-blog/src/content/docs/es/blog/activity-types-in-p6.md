...
Muchos planificadores se enfocan primero en nombres de actividades, duraciones, fechas y relaciones. Eso es esencial, pero el tipo de actividad tambien importa. Una task activity, un milestone, una actividad Level of Effort y una WBS Summary no se comportan igual. Elegir el tipo incorrecto puede distorsionar fechas, progreso, recursos, float y reportes.
...
Si el tipo de actividad no coincide con el proposito, el cronograma puede volverse confuso. Un milestone con duracion no es realmente un milestone. Una task normal usada como resumen puede ocultar logica. Una actividad Level of Effort usada para impulsar trabajo puede distorsionar la ruta critica. Una Resource Dependent usada incorrectamente puede calcular distinto a lo esperado.
...
Esto puede ser util cuando la disponibilidad de recursos es un driver real del trabajo. Por ejemplo, una cuadrilla especializada, un inspector o un equipo especifico puede estar disponible solo ciertos dias o turnos.
...
Use un Start Milestone cuando el cronograma necesita marcar el comienzo de algo importante. Normalmente debe estar conectado con logica que explique que impulsa el milestone y que trabajo libera.
...
Los Finish Milestones son utiles para reporte porque marcan logro. No deben usarse como actividades normales de trabajo. Si se requiere esfuerzo para alcanzar el milestone, ese esfuerzo debe modelarse como tareas que llegan al milestone.
...
Una actividad LOE normalmente deriva sus fechas de otras actividades. Debe representar esfuerzo de soporte que continua mientras ocurre otro trabajo. Normalmente no esta pensada para ser driver de tareas discretas de construccion o ingenieria.
...
El tipo de actividad debe hacer que el cronograma sea mas facil de entender. Si los revisores necesitan preguntar por que un milestone tiene duracion, por que un LOE impulsa trabajo o por que una WBS Summary aparece en logica detallada, el tipo de actividad puede estar mal.

## Errores Comunes

Un error comun es usar milestones como sustitutos de trabajo. Un milestone debe marcar un punto en el tiempo. Si se requiere trabajo, cree actividades para ese trabajo.
