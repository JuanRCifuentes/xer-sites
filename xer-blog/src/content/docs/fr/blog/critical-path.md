...
Cette distinction est importante. Une activité de grande valeur peut ne pas être critique si elle est flottante. Un jalon visible pour le client peut ne pas être critique si un autre chemin mène à son achèvement. Une petite activité technique peut être critique si elle se situe dans la seule chaîne menant au transfert final.
...
Si le chemin commence par une activité qui n'a aucune logique de conduite, un début de date de données inexpliqué ou un jalon douteux, l'examinateur doit enquêter. Le calendrier montre peut-être un chemin calculé, mais pas crédible.
