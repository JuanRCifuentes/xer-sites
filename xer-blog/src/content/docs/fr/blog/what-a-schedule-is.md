...
flowchart LR
    ...
    E --> F["Jalons, chemin critique, flottant, histogrammes, courbes et flux de trésorerie"]
```
...
Float est un autre résultat important. Float indique le degré de flexibilité d'une activité avant qu'elle n'affecte une autre activité ou la fin du projet. Mais float n'a de sens que lorsque le réseau horaire est complet. Si les activités manquent de logique, le flottement peut paraître meilleur ou pire que la réalité.

## Pourquoi la logique rend la chronologie crédible

C'est là que la métrique « Activités commençant à la date des données sans logique de conduite » devient importante.
