...
Ces dates ne signifient pas toutes la même chose. Certains sont calculés par la logique CPM. Certains sont saisis lors des mises à jour de progression. Certains sont utilisés à titre de comparaison. Certains sont utilisés pour contrôler ou limiter le planning. Comprendre la différence est essentiel pour la qualité du calendrier, les rapports PMO, la préparation à l'analyse des retards et le contrôle de base du projet.
...
Si une activité a des dates réelles postérieures à la date des données, il s'agit généralement d'une erreur de statut. Si une activité ouverte commence exactement à la date des données sans logique de conduite, cela peut indiquer un séquençage manquant. Si une fin attendue est antérieure à la date des données, cela peut indiquer des informations de mise à jour obsolètes.
...
Les dates tardives font partie du passage en arrière. Ils sont utilisés pour calculer le float. La différence entre les dates anticipées et tardives permet de montrer la flexibilité de l’activité.
...
Par exemple, BL1 Start et BL1 Finish peuvent afficher les dates de début et de fin de l'activité par rapport à la référence approuvée. Le début et la fin actuels affichent les dernières prévisions. La différence entre eux montre la variance.
...
- Utilisez les dates anticipées et tardives pour comprendre le calcul et la marge flottante du CPM.
...
Une troisième erreur consiste à ignorer l’heure de la journée. P6 stocke les dates avec l'heure, et les différences de calendrier peuvent créer des décalages apparents d'un jour ou des surprises flottantes.
