...
- Atividades críticas são atividades com flutuação zero ou negativa.
...
A primeira dimensão é a sensibilidade ou probabilidade do cronograma. Isso pode ser medido pela frequência com que uma atividade se torna crítica durante a simulação de Monte Carlo, ou pelo quão próxima ela está do ponto crítico com base na flutuação total ou em limites quase críticos.
...
Métricas de risco importantes incluem Índice de Criticidade, Flutuação Total, Índice de Sensibilidade ao Cronograma e duração ou valor de impacto.
...
Total Float mostra o quão perto uma atividade está de afetar o término do projeto no cronograma determinístico. A flutuação próxima de zero é um sinal de alerta.
...
| Atividade | Flutuador | Índice de criticidade | Duração | Resultado da Matriz |
| --- | ---: | ---: | ---: | --- |
| UM | 0 dias | 95% | 20 dias | Foco crítico |
| B | 5 dias | 60% | 15 dias | Alta prioridade |
| C | 20 dias | 15% | 10 dias | Monitor |

A atividade A pertence à área de alta criticidade e alto impacto. Não tem flutuação, parece crítico na maioria das simulações e tem longa duração. Merece controle focado.

A Atividade B pode não ser tão urgente quanto a Atividade A, mas ainda assim merece atenção. Tem flutuação limitada e uma probabilidade significativa de se tornar crítica.

A atividade C tem mais flutuação e menor criticidade. Não deve ser ignorado, mas não requer o mesmo nível de foco de gestão.
...
Comece exportando dados de atividades do P6. Inclua ID da atividade, nome da atividade, EAP, flutuação total, duração restante, início, término, calendário, restrições e indicadores de caminho crítico ou mais longo.

Em seguida, adicione campos opcionais de análise de risco, como Índice de Criticidade e Índice de Sensibilidade do Cronograma. Se os dados de simulação não estiverem disponíveis, utilize limites práticos baseados na flutuação e na duração. Por exemplo, alta criticidade pode significar folga total menor ou igual a 0 dias, ou IC acima de 70%. A criticidade média pode significar flutuação quase crítica ou CI entre 40% e 70%.
