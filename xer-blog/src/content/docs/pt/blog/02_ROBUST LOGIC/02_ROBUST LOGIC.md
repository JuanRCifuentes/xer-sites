...
A métrica "Atividades iniciadas na data dos dados sem lógica direcionadora" é um forte teste de qualidade do cronograma.

A Data Date é o limite entre o desempenho real e o trabalho previsto. Quando uma atividade começa exatamente na Data Date, o revisor deve fazer uma pergunta simples: o que está impulsionando esse início?
...
Se a atividade não tiver lógica direcionadora, a largada é mais fraca. A atividade pode estar na Data de Dados porque não tem antecessor, porque a lógica está incompleta, porque uma restrição a está forçando ou porque a atualização não foi totalmente atualizada.
...
Pouca lógica cria inícios abertos, finais abertos, folga não confiável e resultados de caminho crítico fracos. Muita lógica pode dificultar a revisão da rede e ocultar o verdadeiro motivador de uma atividade.
...
Quando falta esse direcionador de início, a atividade pode mover-se para uma posição artificial no cronograma. Durante as atualizações, pode aparecer na Data Date. Isso cria uma falsa sensação de prontidão.

Considere uma atividade chamada “Instalar Bombas”. Se começar na Data Date, mas não tiver antecessor para conclusão da fundação, entrega da bomba ou transferência de área, o cronograma não explica por que a instalação pode começar. A atividade pode ser planeada, mas a lógica não é robusta.
...
flowchart TD
    ...
    F -- "Não" --> G["Revise o direcionador de início ausente"]
...
No P6, colunas de revisão úteis incluem ID da atividade, nome da atividade, EAP, início, término, status da atividade, folga total, predecessores, sucessores, tipo de relacionamento, atraso, restrições, calendário e indicadores de relacionamento de condução, se disponíveis.

Para cada atividade iniciada na Data Date, pergunte:
...
Quando as atividades começam na Data Date sem nenhuma lógica direcionadora, o cronograma expõe uma fraqueza nessa história. A atividade pode ser mostrada como pronta, mas a rede não explica o porquê.
...
- [Dependências ausentes no Primavera P6 - Visão geral](../../metrics/21_missing_dependencies/02_guide_template.md)
- [Lógica redundante em programações do Primavera P6 - Visão geral](../../metrics/06_redundant_logic/02_guide_template.md)
- [O que é um cronograma](../01_WHAT%20A%20SCHEDULE%20IS/01_blog.md)
