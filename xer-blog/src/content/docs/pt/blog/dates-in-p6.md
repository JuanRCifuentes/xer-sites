...
flowchart TD
    ...
    C --> H["Agendar revisão e relatórios"]
    E --> H
    G --> H
```
...
A Data dos Dados não é uma data de atividade, mas controla como todas as datas de atividade devem ser interpretadas.

A Data dos Dados é o limite entre o desempenho real e o trabalho previsto. O trabalho antes da Data dos Dados deve ser atualizado ou status. O trabalho após a data dos dados deve ser previsto.

Se uma atividade tiver datas reais posteriores à Data dos Dados, isso geralmente é um erro de status. Se uma atividade aberta começar exatamente na data dos dados sem nenhuma lógica de condução, isso pode indicar falta de sequenciamento. Se o término esperado for anterior à data dos dados, isso pode indicar informações de atualização desatualizadas.

Antes de revisar qualquer data de atividade, confirme a Data dos Dados.
...
As datas atrasadas fazem parte do retrocesso. Eles são usados ​​para calcular o float. A diferença entre as datas iniciais e tardias ajuda a mostrar quanta flexibilidade a atividade possui.
...
As datas reais normalmente devem ser iguais ou anteriores à Data dos Dados. Se as datas reais forem posteriores à Data dos Dados, o cronograma reporta o trabalho futuro como já iniciado ou concluído, o que enfraquece a credibilidade da atualização.
...
Acabamento Esperado pode ser útil para trabalhos em andamento quando as equipes de campo fornecem uma expectativa de finalização realista. Mas se não for mantido, pode ficar obsoleto. Um término esperado antes da data dos dados é um sinal de alerta comum.
...
- Use datas iniciais e finais para entender o cálculo e a flutuação do CPM.
...
- Use a Data dos Dados para separar o desempenho real do trabalho previsto.
...
Um terceiro erro é ignorar a hora do dia. P6 armazena datas com hora, e diferenças de calendário podem criar aparentes turnos de um dia ou surpresas flutuantes.
