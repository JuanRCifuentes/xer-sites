...
진행 상황은 단순한 보고 수치가 아니기 때문에 올바른 것을 선택하는 것이 중요합니다. 이는 남은 기간, 획득 가치, 자원 보고, 일정 신뢰성 및 각 업데이트 주기의 품질에 영향을 미칩니다.
...
활동에서 기간 완료율(%)을 사용하는 경우 활동 완료율은 원래 기간, 실제 기간, 남은 기간 간의 관계에 따라 결정됩니다.
...
시간이 진행 상황을 공정하게 측정하고 남은 기간이 주의 깊게 유지되는 경우 기간 % 완료를 사용합니다.
...
완료율과 남은 기간은 일관된 내용을 전달해야 합니다.

활동은 물리적으로 80% 완료될 수 있지만 남은 작업이 어렵거나 지연되거나 다른 조건에 따라 달라지는 경우 남은 기간은 여전히 ​​10일입니다. 그것은 유효할 수 있습니다.

계획된 시간의 절반이 경과했기 때문에 활동은 50% 기간 % 완료일 수 있지만 작업의 20%만 물리적으로 완료된 경우 남은 기간을 수정해야 할 수 있습니다.

그렇기 때문에 좋은 업데이트에는 진행 상황과 예측 검토가 모두 필요합니다. 완료율은 얼마나 달성되었는지를 나타냅니다. 남은 기간은 아직 필요한 시간을 나타냅니다.
...
또 다른 문제는 완료율을 업데이트하지만 남은 기간을 무시하는 것입니다. 활동은 진행 상황을 표시하지만 여전히 비현실적인 예측을 가질 수 있습니다.

## 모범 사례

업데이트 주기가 시작되기 전에 진행 규칙을 정의합니다. 프로젝트 팀은 어떤 활동 그룹이 기간, 실제 또는 단위 완료율을 사용하는지 알아야 합니다.

완료율 유형, 활동 완료율(%), 물리적 완료율(%), 기간 완료율(%), 단위 완료율(%), 남은 기간, 실제 시작, 실제 완료 및 활동 상태를 표시하는 레이아웃을 사용합니다.

다음과 같은 불일치를 확인하십시오.

- 진행률 0%로 활동을 시작했습니다.
- 남은 기간 = 0이지만 상태가 완료되지 않았습니다.
- 실제 완료 없이 100% 진행됩니다.
...
강력한 일정은 의도적으로 완료율 유형을 사용합니다. 방법이 작업과 일치하면 진행 상황 업데이트가 더 명확해지고 남은 기간이 더 안정적이 되며 프로젝트 보고를 방어하기가 더 쉬워집니다.
