...
如果活动使用物理完成百分比，则活动完成百分比将遵循用户输入的物理完成百分比值。
...
风险在于花费的时间并不总是等于完成的工作。一项任务可能会消耗其计划工期的一半，但在物理上仍然远远落后。如果调度程序仅依赖于持续时间，进度报告可能看起来比实际情况更好。

## 物理完成百分比

物理完成百分比是根据实际物理进度手动输入或更新的。它代表了工作中真正取得的成就，与持续时间或资源单位无关。
...
好处是它比过去的时间更能反映现实。风险在于它需要纪律。项目团队必须定义如何衡量身体进步、由谁批准以及如何收集证据。
...
相似活动组的选择应该保持一致。工程可交付成果可以使用物理完成百分比。施工安装可以根据数量使用物理完成百分比。基于时间的管理支持可以使用持续时间完成百分比。如果资源数据可靠，资源密集型工作包可以使用完成单位百分比。
...
这就是为什么好的更新需要进度和预测审查。完成百分比表明已经实现了多少。剩余持续时间表明还需要多少时间。

## 常见错误

一个常见的错误是对身体进步与时间不成比例的活动使用“完成时间百分比”。这可能会使进展看起来比实际工作更好或更糟。

另一个错误是在没有测量规则的情况下使用物理完成百分比。如果一个学科按安装数量报告实际进度，而另一个学科按意见报告，则进度表会变得不一致。
...
P6 中的完成百分比类型定义了如何衡量活动进度。持续时间完成百分比衡量基于时间的进度。物理完成百分比衡量实际完成的工作。单位完成百分比衡量资源单位进度。

没有哪一种类型适合所有活动。正确的选择取决于工作的计划、衡量和控制方式。

强大的时间表有意使用百分比完成类型。当方法与工作相匹配时，进度更新变得更加清晰，剩余工期变得更加可靠，项目报告变得更容易辩护。
