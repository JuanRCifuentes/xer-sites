...
如果时间表规定一名工作人员必须同时从事五项活动，P6 可以显示该需求。但如果没有资源限制，时间表就无法清楚地表明该需求是否现实。该限制允许计划者看到超载、容量问题以及可能的资源驱动的进度问题。
...
资源限制很重要，因为时间表通常在技术上是可行的，但实际上是不可能的。
...
在项目控制中，当进度表用于人员配备、采购支持、施工规划或挣值报告时，这一点尤其有价值。
...
这并不意味着时间表是错误的。这意味着规划者必须审查该计划。解决方案可能是增加人员、改变顺序、转移非关键工作、使用加班时间，或者在现实且经批准的情况下接受临时高峰。
...
材料分配可以显示计划的混凝土体积、电缆长度、钢材吨位或安装数量。项目可能仍然存在材料限制，但这些限制通常是通过采购日期、交付里程碑、库存跟踪或时间表中的限制来管理的，而不是通过用于人员或设备的相同类型的每日资源可用性限制来管理。

这并不意味着材料不重要。这意味着规划者应该小心该限制所代表的含义。
...
因此，资源限制应该是受控调度过程的一部分，而不是更新结束时按下的按钮。
...
资源限制在假设分析中也很有用。规划人员可以测试当前计划是否适用于可用容量，或者是否需要额外的人员、加班或重新排序。
...
该限制应与实际规划问题相符。我们是在测试实际的人员可用性、预算人员配置、设备访问权限还是管理目标？每一个可能需要不同的设置。

## 常见错误

一个常见的错误是在没有就资源限制所代表的含义达成一致的情况下设置资源限制。资源可能显示每天 80 小时，但是当前人员、最大人员、预算人员还是承包商承诺的人员？

另一个错误是使用调平结果而不进行审查。 P6 可以根据资源规则移动活动，但规划者仍必须检查结果是否具有构造意义。
...
运用得当，资源限制有助于识别过载、支持人力规划、控制设备需求并提高进度的现实性。如果使用不当，它们可能会产生误导性的直方图或人为的水平结果。
