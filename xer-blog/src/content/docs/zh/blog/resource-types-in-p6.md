...
时间表可以在没有资源的情况下存在，但加载资源的时间表可以为项目团队提供更深入的了解。它可以显示劳动力直方图、设备需求、材料使用、成本曲线、资源限制和可能的超载。为了使该信息有用，调度程序必须了解 P6 中可用的不同资源类型以及何时使用每种资源类型。
...
P6还使用角色，角色与资源并不完全相同，但密切相关，并且在规划时非常有用。
...
例如，一项名为“安装电缆桥架”的活动可能需要 4 名电工进行 5 天。分配劳动力资源可以让时间表显示该期间对电工的需求。
...
角色是通用的工作职能或技能类别。它们与资源不同，但它们有助于在已知命名资源之前进行规划。
...
角色在早期规划中非常有用，因为项目可能知道需要什么类型的技能，但不确切知道谁将执行这项工作。
...
随着项目的成熟，应该对角色进行审查。如果日程需要详细控制，则可能需要用实际资源替换角色。
...
另一个问题是角色和资源的混淆。角色有利于规划，但当详细的分配、日历和实际情况很重要时，命名资源会更好。
...
P6 中的资源类型有助于定义执行工作所需的资源。劳动力资源代表人和船员。非劳动力资源代表设备和可重复使用的资产。物质资源代表消耗数量。在已知指定资源之前，角色支持按技能或功能进行规划。

选择正确的资源类型可以使计划更易于分析。它改进了劳动力直方图、设备规划、材料跟踪、成本负荷、挣值和预测报告。

一个好的资源加载时间表不仅仅是附有资源的时间表。这是一个时间表，其中每种资源类型都被有意使用并在项目的整个生命周期中得到维护。
