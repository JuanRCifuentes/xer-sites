...
关键路径讲述了进度计划当前的确定性故事。关键性矩阵更进一步。它可以帮助团队了解哪些活动已经很关键，哪些活动即将变得关键，以及哪些活动一旦失误就会造成严重影响。

这很重要，因为今天至关重要的活动并不总是唯一值得关注的活动。具有高延迟影响的近乎关键活动可能会成为明天的问题。长期采购活动可能不在当前的关键路径上，但它可能会带来足够的风险来证明严密控制是合理的。

## P6 中的关键性意味着什么

在Primavera P6中，关键性通常是指一项活动如果延迟是否会影响项目完成日期。传统上，P6 使用总浮时或最长路径设置来识别关键活动。
...
该定义很有用，但并不完整。它基于一种计算出的进度计划条件。它不能完全解释不确定性、概率或活动失败时影响的大小。
...
第一个维度是计划敏感性或可能性。这可以通过蒙特卡罗模拟期间活动变得关键的频率来衡量，或者根据总浮时或接近关键阈值来衡量它与关键的接近程度。
...
活动 A 属于高关键性和高影响范围。它没有浮时，在大多数模拟中显得至关重要，并且持续时间很长。它值得集中控制。

活动 B 可能不如活动 A 紧急，但仍然值得关注。它的流通量有限，并且成为关键的可能性很大。

活动 C 具有更多的浮时性和较低的关键性。它不应该被忽视，但它不需要同等程度的管理重点。

## 为什么它有用

关键性矩阵有助于项目团队避免仅依赖单一确定性关键路径。确定性路径很重要，但这只是进度计划的一种视图。
...
首先从 P6 导出活动数据。包括活动 ID、活动名称、WBS、总浮时、剩余持续时间、开始、完成、日历、约束以及关键或最长路径指示器。

然后添加可选的风险分析字段，例如关键性指数和进度敏感度指数。如果没有可用的模拟数据，请使用基于浮时和持续时间的实用阈值。例如，高关键性可能意味着总浮时小于或等于 0 天，或 CI 高于 70%。中等临界可能意味着近临界浮时或 CI 在 40% 到 70% 之间。
...
另请记住，该矩阵取决于进度质量。如果 P6 进度计划缺少逻辑、不切实际的持续时间、硬约束、糟糕的日历或弱状态更新，则矩阵将继承这些弱点。

矩阵的最佳用途是将分析输出与专业进度计划判断相结合。

## 结论

关键性矩阵根据项目活动变得关键的可能性以及延迟时产生的影响程度对项目活动进行排名。它使用P6数据，如总浮时、持续时间、约束和逻辑，并可以通过蒙特卡罗结果（如关键性指数和进度敏感度指数）进行强化。
