...
في جدول بريمافيرا P6 الجيد، المنطق ليس زخرفة. وهو المحرك الذي يسمح للجدول بحساب التواريخ والسماحية الزمنية والمسار الحرج وحركة التنبؤ. إنه يروي قصة التنفيذ بطريقة يمكن مراجعتها وتحديها وتحسينها.
...
يعد مقياس "الأنشطة التي تبدأ في تاريخ البيانات بدون المنطق المحرك" اختبارًا قويًا لجودة الجدول الزمني.
...
يؤدي القليل جدًا من المنطق إلى إنشاء بدايات مفتوحة، وتشطيبات مفتوحة، والسماحية الزمنية غير موثوق، ونتائج مسار حرج ضعيفة. الكثير من المنطق يمكن أن يجعل مراجعة الشبكة صعبة ويمكن أن يخفي المحرك الحقيقي للنشاط.
...
- عادةً ما يجب أن يكون للنشاط الذي يتبعه النشاط اللاحق SS أيضًا النشاط اللاحق FF أو FS.
- عادةً ما يكون للنشاط الذي له النشاط السابق FF النشاط السابق SS أو FS.

يساعد هذا في منع ربط الأنشطة على جانب واحد فقط من مدتها. يجب أن يوضح الجدول كيفية بدء العمل وكيفية اكتماله.

```mermaid
flowchart TD
    A["النشاط مع النشاط اللاحق SS فقط"] --> B{"هل لديها أيضًا النشاط اللاحق FF أو FS؟"}
    B -- "لا" --> C["مراجعة المسار المنطقي غير المكتمل"]
    B -- "نعم" --> D["منطق لاحق متصل بشكل أفضل"]
    E["النشاط مع النشاط السابق FF فقط"] --> F{"هل لديها أيضًا النشاط السابق SS أو FS؟"}
    F -- "لا" --> G["مراجعة برنامج التشغيل المفقود"]
    F -- "نعم" --> H["منطق النشاط السابق متصل بشكل أفضل"]
```
...
في P6، تتضمن أعمدة المراجعة المفيدة معرف النشاط، واسم النشاط، وWBS، والبدء، والانتهاء، وحالة النشاط، وإجمالي السماحية الزمنية، والأسلاف، والخلفاء، ونوع العلاقة، والتأخر، والقيود، والتقويم، ومؤشرات العلاقة المحركة إذا كانت متوفرة.

لكل نشاط يبدأ في تاريخ البيانات، اسأل:

- هل النشاط جاهز حقًا للبدء؟
- أي النشاط السابق يسمح بالبدء؟
- هل هذا النشاط السابق كامل أم قيد التنفيذ أم متوقع؟
- هل العلاقة تقود؟
...
عندما تبدأ الأنشطة في تاريخ البيانات بدون المنطق المحرك، فإن الجدول يكشف عن نقطة ضعف في تلك القصة. قد يظهر النشاط على أنه جاهز، لكن الشبكة لا توضح السبب.
