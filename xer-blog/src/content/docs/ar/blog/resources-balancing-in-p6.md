...
في الجدولة اليومية، غالبًا ما يستخدم الأشخاص عبارة "موازنة الموارد" و"تسوية الموارد" كما لو أنها تعني نفس الشيء. إنهما مرتبطان، لكنهما ليسا متماثلين تمامًا.
...
تعد تسوية الموارد إحدى ميزات P6 التي يمكنها نقل الأنشطة بناءً على توفر الموارد وإعدادات التسوية.
...
flowchart TD
    ...
    F --> I["استخدم تسوية الموارد بعناية"]
    G --> J["التحقق من صحة مواعيد التوقعات"]
    H --> J
    I --> J
```
...
## موازنة الموارد مقابل تسوية الموارد

موازنة الموارد هي نشاط الإدارة والتخطيط.

تسوية الموارد هي عملية حسابية للجدولة.

وهذا التمييز مهم. يمكن للمخطط موازنة الموارد يدويًا من خلال مراجعة الرسوم البيانية وضبط الجدول الزمني بناءً على معرفة المشروع. يمكن أن تساعد تسوية الموارد P6 أيضًا عن طريق تأخير الأنشطة تلقائيًا عندما يتجاوز الطلب على الموارد مدى التوفر.

كلا النهجين يمكن أن يكونا مفيدين.

يكون التوازن اليدوي أفضل عندما يحتاج المجدول إلى الحكم، أو المدخلات الميدانية، أو مراجعة قابلية الإنشاء، أو التحكم الدقيق في الأنشطة التي تتحرك.

تكون تسوية الموارد P6 مفيدة عندما تكون بيانات الموارد موثوقة، ويتم تحديد حدود الموارد، وتكون التقويمات صحيحة، ويريد المجدول اختبار كيفية تغير الجدول عند فرض توفر الموارد.

لا ينبغي أن يحل التسوية محل الحكم التخطيطي. وينبغي أن تدعم ذلك.

## ما يحتاجه P6 قبل التسوية

قبل استخدام ميزة تسوية الموارد P6، يجب أن يكون الجدول جاهزًا لتحليل الموارد.
...
على سبيل المثال، إذا تم تعيين كل أعمال البناء لمورد "طاقم بناء" عام واحد، فقد يُظهر P6 تحميلًا زائدًا للمورد، ولكن النتيجة قد لا تخبر المشروع ما إذا كانت المشكلة مدنية أم مواسير أم كهربائية أم ميكانيكية. يجب أن يتطابق إعداد المورد مع قرار التخطيط.

## كيف يستخدم P6 تسوية الموارد

يقوم نظام تسوية الموارد P6 بمراجعة تعيينات الموارد ومدى توفرها. اعتمادًا على الإعدادات، قد يؤدي ذلك إلى تأخير الأنشطة لتقليل التخصيص الزائد للموارد أو إزالته.

يمكن أن يأخذ الحساب في الاعتبار حدود الموارد ومنطق النشاط والتعويم والتقويمات والأولويات وخيارات التسوية. تعتمد النتيجة الدقيقة على كيفية تكوين المشروع.
...
إذا لم يتم تحديث الوحدات الفعلية، فقد تنحرف منحنيات الموارد بعيدًا عن الواقع. إذا تم تعيين الموارد فقط لتحميل التكلفة، فقد لا تمثل الوحدات القدرة الحقيقية. إذا كانت التقويمات خاطئة، فقد يكون توفر الموارد خاطئًا أيضًا.

كن حذرًا أيضًا عند استخدام تسوية الموارد وفقًا لجدول تعاقدي أو أساسي. التسوية يمكن أن تحرك التواريخ وتؤثر على التعويم. يجب أن يفهم الفريق ما إذا كان الجدول الزمني المستوي هو الخطة الرسمية، أو سيناريو ماذا لو، أو عرض التخطيط الداخلي.
...
ثم قم بمراجعة ملف تعريف المورد أو الرسم البياني في صفحة 6. ابحث عن القمم والأحمال الزائدة والفجوات والتغيرات المفاجئة في الطلب.
...
احتفظ بنسخة من الجدول غير المستوي قبل تشغيل تسوية الموارد. وهذا يعطي الفريق نقطة مرجعية ويساعد في شرح ما تغير.
...
إعدادات تسوية المستندات عند استخدام ميزة P6. يمكن أن تؤدي تسوية الموارد إلى نتائج مختلفة اعتمادًا على الخيارات المحددة، وبالتالي تكون الإعدادات جزءًا من سجل الجدول.
...
يمكن أن تدعم تسوية الموارد P6 هذه المراجعة عن طريق نقل الأنشطة بناءً على توفر الموارد، ولكن يجب استخدامها بعناية ومراجعتها بعد الحساب.
