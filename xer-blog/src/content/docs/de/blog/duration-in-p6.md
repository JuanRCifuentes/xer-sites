...
P6 speichert die Dauer als Zeit, üblicherweise auf Stundenebene in den zugrunde liegenden Zeitplandaten. Was der Benutzer im Layout sieht, kann je nach Präferenz als Tage, Wochen, Monate oder Jahre angezeigt werden.
...
Aus diesem Grund können zwei Personen denselben Zeitplan betrachten und verwirrt sein, wenn ihre Benutzerpräferenzen oder administrativen Zeitintervalleinstellungen nicht übereinstimmen.

## Dauer und Kalender

Kalender teilen P6 mit, wann gearbeitet werden kann. Die Dauer teilt P6 mit, wie viel Arbeitszeit benötigt wird. Der Kalender ordnet diese Arbeitszeit dann den tatsächlichen Daten zu.
...
Die verbleibende Dauer ist die Menge an Arbeitszeit, die noch benötigt wird, um die Aktivität ab dem aktuellen Datendatum abzuschließen.
...
Die tatsächliche Dauer stellt die Zeit dar, die basierend auf dem tatsächlichen Fortschritt bereits für die Aktivität aufgewendet wurde. Es ist an den tatsächlichen Start, das tatsächliche Ende, das Datendatum, Kalender und die Aktualisierungsmethode gebunden.

Die tatsächliche Dauer sollte die Statusgeschichte unterstützen. Wenn eine Aktivität begonnen hat, sollte die tatsächliche Dauer im Verhältnis zum tatsächlichen Start- und Datendatum sinnvoll sein. Wenn die Aktivität abgeschlossen ist, sollte die tatsächliche Dauer mit der tatsächlichen Arbeitsperiode übereinstimmen.
...
Dies wirkt sich auf das aus, was der Benutzer sieht, nicht unbedingt auf die zugrunde liegende Zeitplanberechnung. Beispielsweise kann die gleiche gespeicherte Dauer in einem Layout als Stunden und in einem anderen als Tage angezeigt werden.
...
Ein drittes Problem sind inkonsistente Benutzereinstellungen. Ein Rezensent sieht möglicherweise Stunden, während ein anderer Tage sieht, und beide denken möglicherweise, dass sich der Zeitplan geändert hat.

Ein weiteres häufiges Problem ist das Ändern der Administratoreinstellungen, nachdem bereits Zeitpläne vorhanden sind. Dies kann dazu führen, dass die angezeigten Dauern unterschiedlich erscheinen, selbst wenn sich die zugrunde liegenden gespeicherten Stunden nicht geändert haben.
...
Die Dauer in P6 ist die Arbeitszeit, nicht nur die verstrichene Kalenderzeit. P6 speichert die Dauer als Zeit, wendet Kalender an, um diese Zeit im Zeitplan zu platzieren, und zeigt sie entsprechend den Benutzerpräferenzen und administrativen Zeitintervalleinstellungen an.
...
Ein guter Planer versteht, dass die Dauer nicht nur eine Eingabe ist. Es ist Teil der Berechnungsmaschine. Wenn Dauer, Kalender und Präferenzen aufeinander abgestimmt sind, wird der Zeitplan einfacher zu erklären und zuverlässiger für die Projektsteuerung.
