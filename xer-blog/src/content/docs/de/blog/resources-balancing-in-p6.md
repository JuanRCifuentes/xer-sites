...
Beim Ressourcenausgleich in Primavera P6 wird der Ressourcenbedarf anhand der verfügbaren Kapazität überprüft und der Plan so angepasst, dass die Arbeit mit den verfügbaren Ressourcen ausgeführt werden kann. Es hilft dem Projektteam zu verstehen, ob der Zeitplan nur logisch korrekt oder auch aus Ressourcensicht sinnvoll ist.
...
Beim Ressourcenausgleich stellt sich eine praktische Frage: Kann das Projekt diesen Zeitplan mit den tatsächlich vorhandenen Ressourcen umsetzen?

Ein Zeitplan kann eine gute Logik, akzeptable Termine und einen angemessenen kritischen Pfad aufweisen. Wenn jedoch dieselbe begrenzte Mannschaft oder Ausrüstung erforderlich ist, um an zu vielen Orten gleichzeitig zu arbeiten, ist der Plan möglicherweise nicht realistisch.
...
```mermaid
flowchart TD
    A["Ressourcenbeladener Zeitplan"] --> B["Überprüfen Sie den Ressourcenbedarf"]
...
Der Ressourcenausgleich ist wichtig, da der Zeitplan die Ausführung und nicht nur die Berechnung unterstützen soll.

Wenn der Plan nächste Woche 50 Schweißer vorsieht, der Auftragnehmer jedoch nur 30 bereitstellen kann, zeigt der Zeitplan einen Bedarf an, der nicht gedeckt werden kann. Wenn zwei kritische Tätigkeiten gleichzeitig denselben Kran erfordern, muss möglicherweise mindestens einer von ihnen umziehen. Wenn für alle technischen Überprüfungsaktivitäten derselbe Spezialist erforderlich ist, kann der Engpass bereits vor Baubeginn auftreten.
...
Der Ressourcenausgleich hilft dabei, den CPM-Zeitplan mit der tatsächlichen Außendienst- und Bürokapazität zu verbinden.
...
Diese Unterscheidung ist wichtig. Ein Planer kann die Ressourcen manuell ausgleichen, indem er Histogramme überprüft und den Zeitplan basierend auf Projektkenntnissen anpasst. Der P6-Ressourcenabgleich kann auch hilfreich sein, indem Aktivitäten automatisch verzögert werden, wenn der Ressourcenbedarf die Verfügbarkeit übersteigt.
...
Der P6-Ressourcenabgleich ist nützlich, wenn die Ressourcendaten zuverlässig sind, Ressourcengrenzen definiert sind, Kalender korrekt sind und der Planer testen möchte, wie sich der Zeitplan ändert, wenn die Ressourcenverfügbarkeit erzwungen wird.

Eine Nivellierung sollte keine Planungsbeurteilung ersetzen. Es sollte es unterstützen.

## Was P6 vor dem Leveln braucht

Bevor Sie die P6-Ressourcennivellierungsfunktion verwenden, sollte der Zeitplan für die Ressourcenanalyse bereit sein.
...
- Der aktuelle Zeitplan wurde gespeichert, sodass das Nivellierungsergebnis verglichen werden kann.
...
Bei der Berechnung können Ressourcengrenzen, Aktivitätslogik, Float, Kalender, Prioritäten und Nivellierungsoptionen berücksichtigt werden. Das genaue Ergebnis hängt von der Projektkonfiguration ab.
...
Dies kann zu einem ressourcenschonenderen Zeitplan führen, kann aber auch den kritischen Pfad ändern, Meilensteine ​​verzögern oder die Arbeit auf eine Weise verschieben, die einer Überprüfung bedarf.
...
Der abgestufte Zeitplan sollte nicht blind akzeptiert werden.
...
Der Ressourcenausgleich ist auch vor der Baseline-Genehmigung nützlich. Eine Ausgangslage, die eine unrealistische Personal- oder Ausrüstungsverfügbarkeit voraussetzt, könnte sich später als schwer zu verteidigen erweisen.
...
Seien Sie auch vorsichtig, wenn Sie den Ressourcenabgleich im Rahmen eines Vertrags- oder Basisplans verwenden. Durch die Nivellierung können Termine verschoben und der Float beeinflusst werden. Das Team sollte verstehen, ob es sich bei dem abgestuften Zeitplan um den offiziellen Plan, ein Was-wäre-wenn-Szenario oder eine interne Planungsansicht handelt.
...
Bewahren Sie eine Kopie des nicht abgeglichenen Zeitplans auf, bevor Sie den Ressourcenabgleich durchführen. Dies gibt dem Team einen Anhaltspunkt und hilft zu erklären, was sich geändert hat.

## Gute Praxis

Nutzen Sie den Ressourcenausgleich als Teil der Zeitplanüberprüfung und nicht als einmalige Aufräumaktion.
...
Erstellen Sie keinen Zeitplan mit schlechter Qualität und erwarten Sie, dass das Ergebnis zuverlässig wird. Zuerst Logik, Kalender, Aktivitätsstatus, verbleibende Dauer und Ressourcenzuweisungen korrigieren.

Einstellungen für die Dokumentnivellierung bei Verwendung der P6-Funktion. Der Ressourcenabgleich kann je nach den ausgewählten Optionen zu unterschiedlichen Ergebnissen führen, daher sind die Einstellungen Teil des Zeitplandatensatzes.
...
Der Ressourcenausgleich in P6 hilft dem Projektteam zu testen, ob der Zeitplan mit den verfügbaren Ressourcen ausgeführt werden kann. Es verbindet Daten und Logik mit Arbeitskräften, Ausrüstung, Fachkräfteverfügbarkeit und tatsächlicher Produktionskapazität.
...
Ein ausgewogener Zeitplan ist nicht unbedingt ein vollkommen reibungsloser Zeitplan. Es handelt sich um einen Zeitplan, bei dem der Ressourcenbedarf sichtbar, realistisch und auf die Art und Weise abgestimmt ist, wie das Projekt tatsächlich umgesetzt wird.
