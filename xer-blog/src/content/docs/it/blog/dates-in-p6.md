...
Queste date non significano tutte la stessa cosa. Alcuni sono calcolati secondo la logica CPM. Alcuni vengono inseriti durante gli aggiornamenti sullo stato di avanzamento. Alcuni sono usati per il confronto. Alcuni vengono utilizzati per controllare o limitare la pianificazione. Comprendere la differenza è essenziale per la qualità della pianificazione, il reporting PMO, la disponibilità dell'analisi dei ritardi e il controllo di base del progetto.

La domanda più importante è semplice: cosa mi dice questa data e da dove viene?

## Perché P6 ha così tante date

P6 non è solo un elenco di date. È un modello di calcolo. Il software calcola le date dalla durata delle attività, dai calendari, dalle relazioni, dai vincoli, dalle risorse, dallo stato di avanzamento e dalla data dei dati.
...
flowchart TD
    ...
    F["Base di riferimento approvata"] --> G["Date di riferimento"]
...
La data dati non è una data di attività, ma controlla il modo in cui devono essere interpretate tutte le date di attività.

La Data Data è il confine tra le prestazioni effettive e il lavoro previsto. Il lavoro prima della data dei dati dovrebbe essere attualizzato o classificato. Dovrebbe essere previsto il lavoro successivo alla data dei dati.

Se un'attività ha date effettive successive alla data dati, in genere si tratta di un errore di stato. Se un'attività aperta inizia esattamente alla data dei dati senza alcuna logica di guida, ciò potrebbe indicare la mancanza di sequenziamento. Se la fine prevista è precedente alla data dati, ciò potrebbe indicare informazioni di aggiornamento obsolete.

Prima di rivedere qualsiasi data di attività, confermare la data dei dati.
...
Se molte attività hanno un inizio anticipato alla data dei dati, il revisore deve verificare se sono veramente pronte o se si tratta di inizi aperti, attività vincolate o attività debolmente collegate.
...
Le date tardive fanno parte del passaggio all'indietro. Sono usati per calcolare il float. La differenza tra le date iniziali e quelle successive aiuta a mostrare quanta flessibilità ha l'attività.
...
Le date effettive dovrebbero normalmente coincidere o precedere la data dei dati. Se le date effettive sono successive alla data dati, la pianificazione segnala il lavoro futuro come già iniziato o completato, il che indebolisce la credibilità dell'aggiornamento.
...
Alcuni utenti si aspettano che le date pianificate mostrino il piano originale per sempre. Questo non è sempre un presupposto sicuro. Per il reporting formale degli scostamenti, una linea di base assegnata è solitamente più affidabile che fare affidamento casualmente sulle date pianificate.

Utilizzare Inizio pianificato e Fine pianificata solo quando la procedura di controllo del progetto definisce chiaramente come vengono mantenuti e cosa significano.

## Inizio previsto e Fine prevista

Le date di base provengono da una pianificazione di base assegnata. Vengono utilizzati per confrontare il programma attuale con il piano approvato.

Ad esempio, Inizio BL1 e Fine BL1 possono mostrare le date di inizio e fine dell'attività dalla previsione approvata. Inizio e Fine attuali mostrano le previsioni più recenti. La differenza tra loro mostra varianza.
...
Finitura prevista può essere utile per il lavoro in corso quando i team sul campo forniscono un'aspettativa di finitura realistica. Ma se non viene mantenuto, può diventare obsoleto. Una fine prevista prima della data dei dati è un segnale di avvertimento comune.
...
Durante la revisione del programma, non guardare solo all'inizio e alla fine. Aggiungere i campi del tipo di vincolo e della data del vincolo al layout. Se le date non si comportano come previsto, i vincoli sono una delle prime cose da controllare.

## Quali date dovresti usare?

Utilizza ogni data per il suo scopo:

- Utilizzare Inizio e Fine per la previsione del programma corrente.
...
- Utilizzare la data dei dati per separare le prestazioni effettive dal lavoro previsto.
...
Le date in P6 sono potenti perché raccontano diverse parti della storia del programma. Le date attuali mostrano la previsione. Le date iniziali e finali spiegano il calcolo del CPM. Le date effettive registrano ciò che è accaduto. Le date di riferimento supportano il confronto. Le date dei vincoli rivelano i controlli imposti. Le date previste possono supportare gli aggiornamenti se mantenute correttamente.

Una revisione approfondita del programma non chiede solo "qual è la data?" Si chiede "che tipo di data è questa, da dove viene ed è credibile?"

Quando il team di progetto comprende il significato di ciascun campo data, la pianificazione diventa più facile da spiegare, più facile da controllare e più affidabile per il controllo del progetto.
